...
function decodeVectorNetwork(
  blobs: any[], blobIdx: number, sx = 1, sy = 1
  ...
  const nV = u32(0), nS = u32(4), nR = u32(8);
  const vOff = 12, sOff = 12 + nV * 12, rOff = 12 + nV * 12 + nS * 28;
  if (rOff > buf.length) return null;
  const verts: { x: number; y: number }[] = [];
  for (let i = 0; i < nV; i++) verts.push({ x: f32(vOff + i * 12 + 4) * sx, y: f32(vOff + i * 12 + 8) * sy });
  const segs: { start: number; end: number; tsx: number; tsy: number; tex: number; tey: number }[] = [];
  for (let i = 0; i < nS; i++) {
    const o = sOff + i * 28;
    segs.push({ start: u32(o + 4), tsx: f32(o + 8) * sx, tsy: f32(o + 12) * sy, end: u32(o + 16), tex: f32(o + 20) * sx, tey: f32(o + 24) * sy });
  }
  ...
  if (nR > 0 && rOff + 8 <= buf.length) {
    ...
    let p = rOff + 4;
    const nLoops = u32(p); p += 4;
    for (let l = 0; l < nLoops && p + 4 <= buf.length; l++) {
      const cnt = u32(p); p += 4;
      const idxs: number[] = [];
      for (let k = 0; k < cnt && p + 4 <= buf.length; k++) { idxs.push(u32(p)); p += 4; }
      loops.push(idxs);
    }
  }
  if (!loops.length) { // fallback: chain segments by shared vertices
    const used = new Set<number>();
    for (let i = 0; i < nS; i++) {
      if (used.has(i)) continue;
      const loop = [i]; used.add(i); let cur = segs[i].end; const startV = segs[i].start;
      while (cur !== startV) {
        let nx = segs.findIndex((s, j) => !used.has(j) && (s.start === cur || s.end === cur));
        if (nx < 0) break;
        used.add(nx); loop.push(nx); cur = segs[nx].start === cur ? segs[nx].end : segs[nx].start;
      }
      loops.push(loop);
    }
  }
  ...
  for (const loop of loops) {
    ...
    for (const si of loop) {
      ...
      const a = cur, b = fwd ? s.end : s.start;
      const ts = fwd ? { x: s.tsx, y: s.tsy } : { x: s.tex, y: s.tey };
      const te = fwd ? { x: s.tex, y: s.tey } : { x: s.tsx, y: s.tsy };
      const c1x = verts[a].x + ts.x, c1y = verts[a].y + ts.y, c2x = verts[b].x + te.x, c2y = verts[b].y + te.y;
      parts.push(`C${n2(c1x)} ${n2(c1y)} ${n2(c2x)} ${n2(c2y)} ${n2(verts[b].x)} ${n2(verts[b].y)}`);
      cur = b;
    }
    parts.push("Z");
  }
...
function pathVisualBBox(d: string, mat: Mat): BBox | null {
  const tx = (x: number, y: number): [number, number] => [mat[0] * x + mat[1] * y + mat[2], mat[3] * x + mat[4] * y + mat[5]];
  let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
  const inc = (x: number, y: number) => { if (x < minX) minX = x; if (y < minY) minY = y; if (x > maxX) maxX = x; if (y > maxY) maxY = y; };
  ...
  const cubicAxis = (p0: number, p1: number, p2: number, p3: number): number[] => {
    const vals = [p0, p3];
    const A = -p0 + 3 * p1 - 3 * p2 + p3, B = 2 * (p0 - 2 * p1 + p2), C = -p0 + p1;
    const roots: number[] = [];
    ...
    for (const t of roots) if (t > 0 && t < 1) { const u = 1 - t; vals.push(u * u * u * p0 + 3 * u * u * t * p1 + 3 * u * t * t * p2 + t * t * t * p3); }
    return vals;
  };
  const quadAxis = (p0: number, p1: number, p2: number): number[] => {
    const vals = [p0, p2]; const den = p0 - 2 * p1 + p2;
    if (Math.abs(den) > 1e-9) { const t = (p0 - p1) / den; if (t > 0 && t < 1) vals.push((1 - t) * (1 - t) * p0 + 2 * (1 - t) * t * p1 + t * t * p2); }
    return vals;
  };
  ...
  while (i < toks.length) {
    const c = toks[i++];
    if (c === "M") { cx = num(); cy = num(); sx = cx; sy = cy; inc(...tx(cx, cy)); }
    else if (c === "L") { cx = num(); cy = num(); inc(...tx(cx, cy)); }
    else if (c === "C") {
      const a1 = num(), b1 = num(), a2 = num(), b2 = num(), x = num(), y = num();
      const [P0x, P0y] = tx(cx, cy), [P1x, P1y] = tx(a1, b1), [P2x, P2y] = tx(a2, b2), [P3x, P3y] = tx(x, y);
      for (const v of cubicAxis(P0x, P1x, P2x, P3x)) { if (v < minX) minX = v; if (v > maxX) maxX = v; }
      for (const v of cubicAxis(P0y, P1y, P2y, P3y)) { if (v < minY) minY = v; if (v > maxY) maxY = v; }
      cx = x; cy = y;
    } else if (c === "Q") {
      const a1 = num(), b1 = num(), x = num(), y = num();
      const [P0x, P0y] = tx(cx, cy), [P1x, P1y] = tx(a1, b1), [P2x, P2y] = tx(x, y);
      for (const v of quadAxis(P0x, P1x, P2x)) { if (v < minX) minX = v; if (v > maxX) maxX = v; }
      for (const v of quadAxis(P0y, P1y, P2y)) { if (v < minY) minY = v; if (v > maxY) maxY = v; }
      cx = x; cy = y;
...
export function extractGeometry(index: SvgIndex, guidKey: string): SvgGeometry {
  ...
  const emit = (n: any, mat: Mat) => {
    ...
    if (!(n.fillGeometry?.length) && !(n.strokeGeometry?.length) && n.vectorData?.vectorNetworkBlob != null) {
      ...
      if (ps.length) {
        ...
        if (vn) for (const p of ps) out.push({ d: vn.d, fill: colorStr(p.color), fillRule: vn.fillRule, opacity: (p.opacity ?? 1) * (n.opacity ?? 1), transform });
      }
    }
  };
  ...
  let natW = Math.ceil(rootNode0?.size?.x ?? 0), natH = Math.ceil(rootNode0?.size?.y ?? 0);
  ...
  const walk = (k: string, mat: Mat, isRoot: boolean, frame: { w: number; h: number }) => {
    ...
    if (n.type !== "FRAME" && n.type !== "SECTION") { emit(n, m); if (frame.w > natW) natW = frame.w; if (frame.h > natH) natH = frame.h; }
    if (n.symbolData?.symbolID) {
      ...
      const mf = master?.size && master.size.x > 0 && master.size.y > 0
        ? { w: Math.ceil(master.size.x), h: Math.ceil(master.size.y) } : frame;
  ...
  let vx = 0, vy = 0, vw = natW || 1, vh = natH || 1;
  if (isFinite(minX)) {
