...
export function describeNode(n: any, opts: DescribeOpts = {}): string {
  ...
  if (n.fontName) bits.push(`font=${n.fontName.family} ${n.fontName.style} ${n.fontSize}px ls=${letterSpacingStr(n.letterSpacing, n.fontSize)} lh=${n.lineHeight?.value !== undefined ? n.lineHeight.value + (n.lineHeight.units === "PERCENT" ? "%" : "px") : "auto"}`);
  ...
  if (n.type === "TEXT") {
    ...
    if (n.textAlignVertical && n.textAlignVertical !== "TOP") bits.push(`valign=${n.textAlignVertical}`);
    if (n.textTruncation && n.textTruncation !== "DISABLED") bits.push(`truncate=${n.textTruncation}`);
  ...
  const composed = n.path !== undefined && Array.isArray(n.children) && n.children.length > 0 && !n.unresolved;
