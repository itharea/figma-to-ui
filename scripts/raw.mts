...
function cmdDump(argv: string[]) {
  ...
  if (!target) throw new Error("usage: raw.mts dump <message.json> <guidKey> [maxDepth] [--abs] [--resolve]");
...
function cmdDiffFrames(argv: string[]) {
  ...
  const a = argv[3];
  const b = argv[4];
  if (!msgPath || !a || !b) throw new Error("usage: raw.mts diff-frames <message.json> <guidA> <guidB>");
