...
| `variables.mts` | `node variables.mts <msg.json>` | design tokens from Figma variables (resolves alias chains transitively → concrete value inline, e.g. `→ 18 (alias Numbers/18)`). **Skips soft-deleted** variables/sets (`isSoftDeleted` — Figma retains deleted tokens in the export for sync/recovery; they are not live tokens) |
...
| `theme-gen.mts` | `node theme-gen.mts <ir-dir> [--framework web\|rn] [--out <dir>]` | turn the IR's complete variable catalog (`tokens/variables.json`) into a typed theme. No `--framework` → emits **both** `theme.ts` (rn) + `theme.css` (web); `--out <dir>` writes the file(s), else prints to stdout. Mirrors Figma's `/`-hierarchy as a tree (`Color/praline/950` → `color.praline['950']`), keyed by **mode** at the top level (union of mode names across collections). **Aliases are emitted as CODE REFERENCES to the direct target**, never collapsed: CSS `--typography-size-m: var(--numbers-18)`; RN a per-mode IIFE declaring each variable as a `const` in **topological order** (a target before its referrer) then returning the nested tree. Numeric CSS vars are unit-less (a px context wraps them: `calc(var(--x) * 1px)`). Pure logic in `theme-lib.mts` (shared with `codegen.mts` so munging never drifts) |
